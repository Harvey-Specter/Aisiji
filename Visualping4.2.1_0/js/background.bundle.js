...
([
  ...
  (function(module, exports, __webpack_require__) {
    ...
    var _servicesVisualDBService = __webpack_require__(195);

    var _servicesVisualDBService2 = _interopRequireDefault(_servicesVisualDBService);
    ...
    var DB2 = new _servicesVisualDBService2['default']();
    DB2.initializeDB();

    new _servicesPickerService2['default'](null).initializeListener();

    // Trigger method from Pop-up since pop-up closes right away on window creation
    var openUpdateAndMarkRead = function openUpdateAndMarkRead(update) {
      try {
        visualJobService.getJobByURL(update.url, function(job) {
          ...
          captureService.captureAndDisplayUpdate({ text_only: textOnly }); // Capture and display update
          new _servicesUpdateService2['default']().remove(update); // Mark as read
        });
      } catch (e) {
    ...
    var openAllUpdates = function openAllUpdates(updates) {
      updates.map(function(update, i) {
        var interval = 2000 * (i + 1);
        setTimeout(function() {
          openUpdateAndMarkRead(update);
        }, interval);
      });
    };

    var pickerInit = function pickerInit(tabID, data) {
      var pickerService = new _servicesPickerService2['default'](tabID);
      pickerService.initialize(data);
    };

    window.openUpdateAndMarkRead = openUpdateAndMarkRead;
    window.openAllUpdates = openAllUpdates;
    window.pickerInit = pickerInit;
      ...
      _createClass(NotificationService, [{
        ...
        value: function triggerRichNotification(update) {
          ...
          var options = {
            ...
            title: title,
            message: 'Click to view website456',
            isClickable: true,
            buttons: buttons
          };
