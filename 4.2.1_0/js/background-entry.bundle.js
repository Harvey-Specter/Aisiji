...
({
        ...
        _createClass(NotificationService, [{
          ...
          value: function triggerRichNotification(update) {
            ...
            var options = {
              ...
              title: title,
              message: 'Click to view website 789',
              isClickable: true,
              buttons: buttons
            };
  ...
  215:
    ...
    (function(module, exports, __webpack_require__) {
      ...
      var _constantsWatchDog = __webpack_require__(212);

      var config = _interopRequireWildcard(_constantsWatchDog);

      var _servicesNotificationService = __webpack_require__(207);

      var _servicesNotificationService2 = _interopRequireDefault(_servicesNotificationService);

      var _servicesPageMonitorToJobsService = __webpack_require__(217);

      var _servicesPageMonitorToJobsService2 = _interopRequireDefault(_servicesPageMonitorToJobsService);

      var _jquery = __webpack_require__(203);

      var _jquery2 = _interopRequireDefault(_jquery);

      var _servicesExtensionVersionService = __webpack_require__(221);

      var _servicesExtensionVersionService2 = _interopRequireDefault(_servicesExtensionVersionService);

      // The following hooks to display were commented out in 4.1.6

      // Display the upgraded page on popup
      // if ( !(['4.1.0', '4.1.1', '4.1.2', '4.1.3', '4.1.4', '4.1.5'].includes(details.previousVersion)) ) {
      //   localStorage.setItem('display_upgraded', true);
      // }

      // if ( !(['4.0.2', '4.0.3', '4.1.0', '4.1.1', '4.1.2', '4.1.3', '4.1.4', '4.1.5'].includes(details.previousVersion)) ) {
      //   // Import jobs only if hasn't migrated yet
      //   pageMonitorToJobsService.importPagesToJobs();
      //   setTimeout(() => {
      //     let isPagemonitorUser = ['true', true].includes(localStorage.getItem('pm_user'));
      //     // console.log('PAGEMONITOR', isPagemonitorUser);
      //     if ( isPagemonitorUser ) {
      //       let url = 'http://visualping.io/pagemonitor';
      //       chrome.windows.create({url: url, type: 'popup', width: 850, height: 740});
      //     }
      //   }, 2000);
      // }

      // Set up Amplitude last
      ...
      var extensionVersionService = new _servicesExtensionVersionService2['default']();
      extensionVersionService.setExtensionVersion();

      var watchdog = new _servicesWatchDog2['default']();
      window.watchdog = watchdog;

      // Uncomment bringUpToDate when this has been factored out to its own module
      // bringUpToDate(parseFloat(getSetting(SETTINGS.version)) || 0, watchdog.check);
      var interval = config.WATCHDOG_INTERVAL;
      setInterval(watchdog, interval);
      new _servicesNotificationService2['default']().notifyAll();

      // Import PM pages for PM users
      var pageMonitorToJobsService = new _servicesPageMonitorToJobsService2['default']();
      (function() {
        if (false) {
          pageMonitorToJobsService.prepareDBForTest();
        }
      })();
      pageMonitorToJobsService.determinePageMonitorUser();

      // Setting defaults if not already set (Migrates from PM settings if found)
      // Timeout until pm_user is set
      setTimeout(function() {
        var isPagemonitorUser = ['true', true].includes(localStorage.getItem('pm_user'));
        // Sounds
        if (!['Enabled', 'Disabled'].includes(localStorage.getItem('sound_enabled'))) {
          if (isPagemonitorUser && localStorage.getItem('sound_alert') === '""') {
            localStorage.setItem('sound_enabled', 'Disabled');
          } else {
            localStorage.setItem('sound_enabled', 'Enabled');
          }
        }
        // Sound clip preference import
        if (localStorage.getItem('sound_clip') === null) {
          if (isPagemonitorUser && localStorage.getItem('sound_alert') !== '""') {
            if (localStorage.getItem('sound_alert').match(/bell/)) {
              localStorage.setItem('sound_clip', 'Chime');
            } else if (localStorage.getItem('sound_alert').match(/cuckoo/)) {
              localStorage.setItem('sound_clip', 'Cuckoo');
            } else {
              localStorage.setItem('sound_clip', 'Ping');
            }
          } else {
            localStorage.setItem('sound_clip', 'Ping');
          }
        ...
        if (localStorage.getItem('desktop_notifications') === null) {
          if (isPagemonitorUser && localStorage.getItem('notifications_enabled') === 'false') {
        ...
        // Tab preference
        if (localStorage.getItem('tab_preference') === null) {
          localStorage.setItem('tab_preference', 'Server');
        }
        // Compare preference
        if (localStorage.getItem('default_compare_preference') === null) {
          localStorage.setItem('default_compare_preference', 'Visual');
        }
      }, 1000);
      ...
      // Set update listener for pop-up
      // Timeout because initial PM determination is async
      chrome.runtime.onInstalled.addListener(function(details) {
        if (details.reason === 'update') {
          // Set previous version in case upgraded message requires it
          localStorage.setItem('previous_version', details.previousVersion);
        }
      });
      setTimeout(function() {
        amplitudeService.setupAmplitude();
        amplitudeService.trackPageMonitorUser();
        amplitudeService.trackAppLaunched();
      }, 1000);
  ...
  217:
    ...
    (function(module, exports, __webpack_require__) {
      ...
      var _PageMonitorDBService = __webpack_require__(218);

      var _PageMonitorDBService2 = _interopRequireDefault(_PageMonitorDBService);
      ...
      var _VisualJobService = __webpack_require__(198);

      var _VisualJobService2 = _interopRequireDefault(_VisualJobService);

      // import { splitBase64Image } from '../formatters/Base64Image';
      // import { intervalToMilliseconds, millisecondsToInterval } from '../formatters/intervalToMilliseconds';

      var _constantsPageMonitorDatabase = __webpack_require__(219);

      var definitions = _interopRequireWildcard(_constantsPageMonitorDatabase);

      var _servicesPageCheck = __webpack_require__(206);

      var PageCheck = _interopRequireWildcard(_servicesPageCheck);

      // Deals with importing old PageMonitor jobs from pages table to visual jobs

      var visualJobService = new _VisualJobService2['default']();
      var pageMonitorToJob = new _formattersPageMonitorToJob2['default']();

      var PageMonitorToJobsService = (function() {
        function PageMonitorToJobsService() {
          _classCallCheck(this, PageMonitorToJobsService);

          this.db = new _PageMonitorDBService2['default']();
        }

        _createClass(PageMonitorToJobsService, [{
          key: 'importPagesToJobs',
          value: function importPagesToJobs() {
            this.db.getAllPages(function(result) {
              // Save Pages to Jobs
              for (var i = 0; i < result.rows.length; i++) {
                var job = pageMonitorToJob.convert(result.rows.item(i));
                visualJobService.insertJob(job);
                // Set the initial CRC for this job
                PageCheck.checkPageHTML(job.url, function() {}, function() {}, true);
              }
              console.log('PM Pages imported');
            });
          }
        }, {
          key: 'determinePageMonitorUser',
          value: function determinePageMonitorUser() {
            var pmUser = false;
            ...
            this.db.getAllPages(function(result) {
              ...
              // If no pages found, then check for pm specific settings
              if (pmUser === false && localStorage.getItem('view_all_action') !== null) {
                pmUser = true;
              }
              // Store setting
              if (pmUser === true) {
                localStorage.setItem('pm_user', true);
              }

              // console.log('PM_USER', localStorage.getItem('pm_user') === 'true');
              return pmUser;
            });
          }

          // Only used to test out import of pages
        }, {
          key: 'prepareDBForTest',
          value: function prepareDBForTest() {
            this.db.executeSql(definitions.PM_DATABASE_PAGES_TABLE_STRUCTURE, []);
            var query = "INSERT INTO pages(url, name, mode, regex, selector, " + "                   check_interval, html, crc, updated, " + "                   last_check, last_changed) " + "VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

            this.db.executeSql(query, ['http://apple.com/ipad', 'Apple', 'text', '', '', 1000 * 60, '', 0, false, 1444163367379, 1444163367379]);
            this.db.executeSql(query, ['http://burton.com/', 'Burton', 'text', '', '', 1000 * 60, '', 0, false, 1444163393883, 1444163393883]);
          }
        }]);

        return PageMonitorToJobsService;
      })();

      exports['default'] = PageMonitorToJobsService;
      module.exports = exports['default'];
  ...
  218:
    ...
    (function(module, exports, __webpack_require__) {
      ...
      var PageMonitorDBService = (function() {
        function PageMonitorDBService() {
          _classCallCheck(this, PageMonitorDBService);
        }

        _createClass(PageMonitorDBService, [{
          key: 'executeSql',
          ...
          value: function executeSql(sql, _args, resultCallback, transactionCallback) {
            ...
            definitions.PM_DATABASE.transaction(transactionFunc, function() {
              return null;
            }, transactionFuncCallback);
          }
        }, {
          key: 'getAllPages',
          value: function getAllPages(resultCallback) {
            var sql = "SELECT * FROM pages;";
            this.executeSql(sql, [], resultCallback);
          }
        }]);

        return PageMonitorDBService;
      })();

      exports['default'] = PageMonitorDBService;
      module.exports = exports['default'];
  ...
  219:
    ...
    (function(module, exports) {
      ...
      var PM_DATABASE = openDatabase('pages', '1.0', 'Monitored Pages', 49 * 1024 * 1024);

      exports.PM_DATABASE = PM_DATABASE;
      // The pages database table structure as an SQL CREATE TABLE statement.
      var PM_DATABASE_PAGES_TABLE_STRUCTURE = "" + "CREATE TABLE IF NOT EXISTS pages ( " + "  `url` TEXT NOT NULL UNIQUE, " + "  `name` TEXT NOT NULL, " + "  `mode` TEXT NOT NULL DEFAULT 'text', " + "  `regex` TEXT, " + "  `selector` TEXT, " + "  `check_interval` INTEGER, " + "  `html` TEXT NOT NULL DEFAULT '', " + "  `crc` INTEGER NOT NULL DEFAULT 0, " + "  `updated` INTEGER, " + "  `last_check` INTEGER, " + "  `last_changed` INTEGER " + ");";
      exports.PM_DATABASE_PAGES_TABLE_STRUCTURE = PM_DATABASE_PAGES_TABLE_STRUCTURE;
  ...
  221:
    ...
    (function(module, exports, __webpack_require__) {
      ...
      var ExtensionVersionService = (function() {
        function ExtensionVersionService() {
          _classCallCheck(this, ExtensionVersionService);

          this.version = null;
        }

        _createClass(ExtensionVersionService, [{
          key: 'setExtensionVersion',
          value: function setExtensionVersion() {
          ...
          value: function getExtensionVersion() {
            var manifest = _jquery2['default'].ajax({
              url: 'manifest.json',
              async: false
            }).responseText;
      ...
      exports['default'] = ExtensionVersionService;
      module.exports = exports['default'];
